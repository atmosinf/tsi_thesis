    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                prednorm = [(float(i)-min(prediction))/(max(prediction)-min(prediction)) for i in prediction]
                negvals = [a * -1 for a in prediction if a < 0]
                posvals = [a for a in prediction if a >= 0]
                for i in prediction:
                    if i < 0:
                        alphavar.append(float(-i)/(max(negvals)-min(negvals)))
                    else:
                        alphavar.append(float(i)/(max(posvals)-min(posvals)))
